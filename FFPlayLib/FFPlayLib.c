...
//---->> FFPlayLib
    ...
    static char dbgStr[256] = {0,};
    #define FFP_LOG(level,...)    {\
                                      sprintf(dbgStr,__VA_ARGS__);\
                                      __sendInfoMessage(level, dbgStr);\
                                  }
...
static int packet_queue_init(PacketQueue *q)
{
    ...
    if (!q->mutex) {
        ...
        FFP_LOG( FFP_INFO_ERROR, "SDL_CreateMutex(): %s\n", SDL_GetError() );
        return AVERROR(ENOMEM);
    }
    q->cond = SDL_CreateCond();
    if (!q->cond) {
        av_log(NULL, AV_LOG_FATAL, "SDL_CreateCond(): %s\n", SDL_GetError());
        FFP_LOG( FFP_INFO_ERROR, "SDL_CreateCond(): %s\n", SDL_GetError() );
        return AVERROR(ENOMEM);
    }
    q->abort_request = 1;
    return 0;
            ...
            do {
                ...
                switch (d->avctx->codec_type) {
                    ...
                    case AVMEDIA_TYPE_AUDIO:
                        ...
                        {
                          bufLen = 0;
                          dataSize = av_get_bytes_per_sample(d->avctx->sample_fmt);
                          bufLen = dataSize * (frame->nb_samples) * (d->avctx->channels);

                          ffp_aud_params.Freq              = frame->sample_rate;
                          ffp_aud_params.Channels          = d->avctx->channels;
                          ffp_aud_params.Format            = dataSize;
                          ffp_aud_params.SamplesInBuffer   = frame->nb_samples;
                          ffp_aud_params.BufferSizeInBytes = bufLen;
                          if (bufLen)
                          {   
                              FFP_events->event_audio( FFP_events->sender, (frame->data), bufLen );
                          }
                        }
                        if (ret >= 0) {
        ...
        if (pkt.data == flush_pkt.data) {
            ...
            if (d->avctx->codec_type == AVMEDIA_TYPE_SUBTITLE) {
                ...
                if (avcodec_send_packet(d->avctx, &pkt) == AVERROR(EAGAIN)) {
                    ...
                    FFP_LOG( FFP_INFO_ERROR, "Receive_frame and send_packet both returned EAGAIN, which is an API violation.\n" );
                    d->packet_pending = 1;
                    av_packet_move_ref(&d->pkt, &pkt);
                }
            }
    ...
    switch (sdl_pix_fmt) {
        case SDL_PIXELFORMAT_UNKNOWN:
            ...
            *img_convert_ctx = sws_getCachedContext(*img_convert_ctx,
            ...
            if (*img_convert_ctx != NULL) {
                ...
                FFP_LOG( FFP_INFO_ERROR, "Cannot initialize the conversion context\n" );
                ret = -1;
            }
            break;
        case SDL_PIXELFORMAT_IYUV:
            if (frame->linesize[0] > 0 && frame->linesize[1] > 0 && frame->linesize[2] > 0) {
                ...
                FFP_LOG( FFP_INFO_ERROR, "Mixed negative and positive linesizes are not supported.\n" );
                return -1;
            }
            break;
        default:
...
static void video_audio_display(VideoState *s)
{
    ...
    if (s->show_mode == SHOW_MODE_WAVES) {
        ...
        if (!s->rdft || !s->rdft_data){
            ...
            FFP_LOG( FFP_INFO_ERROR, "Failed to allocate buffers for RDFT, switching to waves display\n" );
    ...
    do {
        ...
        if (got_frame) {
                ...
                if (reconfigure) {
                    ...
                    FFP_LOG( FFP_INFO_DEBUG, "Audio frame changed from rate:%d ch:%d fmt:%s layout:%s serial:%d to rate:%d ch:%d fmt:%s layout:%s serial:%d\n",
                           is->audio_filter_src.freq, is->audio_filter_src.channels, av_get_sample_fmt_name(is->audio_filter_src.fmt), buf1, last_serial,
                           frame->sample_rate, frame->channels, av_get_sample_fmt_name(frame->format), buf2, is->auddec.pkt_serial );
...
static int decoder_start(Decoder *d, int (*fn)(void *), void *arg)
{
    ...
    if (!d->decoder_tid) {
        ...
        FFP_LOG( FFP_INFO_ERROR, "SDL_CreateThread(): %s\n", SDL_GetError() );
        return AVERROR(ENOMEM);
    }
    return 0;
}
...
static int video_thread(void *arg)
{
    ...
    for (;;) {
        ...
        if (   last_w != frame->width
            ...
            FFP_LOG( FFP_INFO_DEBUG, "Video frame changed from size:%dx%d format:%s serial:%d to size:%dx%d format:%s serial:%d\n",
                   last_w, last_h,
                   (const char *)av_x_if_null(av_get_pix_fmt_name(last_format), "none"), last_serial,
                   frame->width, frame->height,
                   (const char *)av_x_if_null(av_get_pix_fmt_name(frame->format), "none"), is->viddec.pkt_serial );
...
static int audio_decode_frame(VideoState *is)
{
    ...
    if (af->frame->format        != is->audio_src.fmt            ||
        ...
        if (!is->swr_ctx || swr_init(is->swr_ctx) < 0) {
            ...
            FFP_LOG( FFP_INFO_ERROR, "Cannot create sample rate converter for conversion of %d Hz %s %d channels to %d Hz %s %d channels!\n",
                    af->frame->sample_rate, av_get_sample_fmt_name(af->frame->format), af->frame->channels,
                    is->audio_tgt.freq, av_get_sample_fmt_name(is->audio_tgt.fmt), is->audio_tgt.channels );                                
            swr_free(&is->swr_ctx);
            return -1;
    ...
    if (is->swr_ctx) {
        ...
        if (out_size < 0) {
            ...
            FFP_LOG( FFP_INFO_ERROR, "av_samples_get_buffer_size() failed\n" );            
            return -1;
        }
        if (wanted_nb_samples != af->frame->nb_samples) {
            if (swr_set_compensation(is->swr_ctx, (wanted_nb_samples - af->frame->nb_samples) * is->audio_tgt.freq / af->frame->sample_rate,
                ...
                FFP_LOG( FFP_INFO_ERROR, "swr_set_compensation() failed\n" );
                return -1;
            }
        }
        ...
        if (len2 < 0) {
            ...
            FFP_LOG( FFP_INFO_ERROR, "swr_convert() failed\n" );
            return -1;
        }
        if (len2 == out_count) {
            av_log(NULL, AV_LOG_WARNING, "audio buffer is probably too small\n");
            FFP_LOG( FFP_INFO_WARNING, "audio buffer is probably too small\n" );
...
static int audio_open(void *opaque, int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate, struct AudioParams *audio_hw_params)
{
    ...
    if (wanted_spec.freq <= 0 || wanted_spec.channels <= 0) {
        ...
        FFP_LOG( FFP_INFO_ERROR, "Invalid sample rate or channel count!\n" );
        return -1;
    }
    ...
    while (!(audio_dev = SDL_OpenAudioDevice(NULL, 0, &wanted_spec, &spec, SDL_AUDIO_ALLOW_FREQUENCY_CHANGE | SDL_AUDIO_ALLOW_CHANNELS_CHANGE))) {
        ...
        FFP_LOG( FFP_INFO_ERROR, "SDL_OpenAudio (%d channels, %d Hz): %s\n", wanted_spec.channels, wanted_spec.freq, SDL_GetError() );
        ...
        if (!wanted_spec.channels) {
            ...
            if (!wanted_spec.freq) {
                ...
                FFP_LOG( FFP_INFO_ERROR, "No more combinations to try, audio open failed\n" );
                return -1;
            }
        }
    ...
    if (spec.format != AUDIO_S16SYS) {
        ...
        FFP_LOG( FFP_INFO_ERROR, "SDL advised audio format %d is not supported!\n", spec.format );
        return -1;
    }
    if (spec.channels != wanted_spec.channels) {
        ...
        if (!wanted_channel_layout) {
            ...
            FFP_LOG( FFP_INFO_ERROR, "SDL advised channel count %d is not supported!\n", spec.channels );
            return -1;
        }
    }
...
static int stream_component_open(VideoState *is, int stream_index)
{
    ...
    if (!codec) {
        if (forced_codec_name)
        {
           ...
           FFP_LOG( FFP_INFO_ERROR, "No codec could be found with name '%s'\n", forced_codec_name );
        }else
        {                   
          av_log(NULL, AV_LOG_WARNING, "No codec could be found with id %d\n", avctx->codec_id);
          FFP_LOG( FFP_INFO_ERROR, "No codec could be found with id %d\n", avctx->codec_id );
        }
        ret = AVERROR(EINVAL);
        goto fail;
    }
    ...
    if(stream_lowres > av_codec_get_max_lowres(codec)){
        ...
        FFP_LOG( FFP_INFO_WARNING, "The maximum value for lowres supported by the decoder is %d\n", av_codec_get_max_lowres(codec) );
    ...
    if ((t = av_dict_get(opts, "", NULL, AV_DICT_IGNORE_SUFFIX))) {
        ...
        FFP_LOG( FFP_INFO_ERROR, "Option %s not found.\n", t->key );
...
static int read_thread(void *arg)
{
    ...
    if (!ic) {
        ...
        FFP_LOG( FFP_INFO_ERROR, "Could not allocate context.\n" );
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    if (find_stream_info) {
        ...
        if (err < 0) {
            ...
            FFP_LOG( FFP_INFO_WARNING, "%s: could not find codec parameters\n", is->filename );
            ret = -1;
            goto fail;
        }
    }
    ...
    if (start_time != AV_NOPTS_VALUE) {
        ...
        if (ret < 0) {
            ...
            FFP_LOG( FFP_INFO_WARNING, "%s: could not seek to position %0.3f\n",is->filename, (double)timestamp / AV_TIME_BASE);
        }
    }
    ...
    for (i = 0; i < AVMEDIA_TYPE_NB; i++) {
        if (wanted_stream_spec[i] && st_index[i] == -1) {
            ...
            FFP_LOG( FFP_INFO_ERROR, "Stream specifier %s does not match any %s stream\n", wanted_stream_spec[i], av_get_media_type_string(i));
            st_index[i] = INT_MAX;
        }
    }
    ...
    if (is->video_stream < 0 && is->audio_stream < 0) {
        av_log(NULL, AV_LOG_FATAL, "Failed to open file '%s' or configure filtergraph\n",
               is->filename);
        FFP_LOG( FFP_INFO_ERROR, "Failed to open file '%s' or configure filtergraph\n",is->filename );
        ret = -1;
        goto fail;
    }
...
//      of the seek_pos/seek_rel variables
            ...
            if (ret < 0) {
                ...
                FFP_LOG( FFP_INFO_ERROR, "%s: error while seeking\n", is->ic->filename ); 
            } else {
                if (is->audio_stream >= 0) {
...
static VideoState *stream_open(const char *filename, AVInputFormat *iformat)
{
    ...
    if (startup_volume < 0)
    {
        ...
        FFP_LOG( FFP_INFO_WARNING, "-volume=%d < 0, setting to 0\n", startup_volume );
    }        
    if (startup_volume > 100)
    {
        av_log(NULL, AV_LOG_WARNING, "-volume=%d > 100, setting to 100\n", startup_volume);
        FFP_LOG( FFP_INFO_WARNING, "-volume=%d > 100, setting to 100\n", startup_volume );
    }
...
static void refresh_loop_wait_event(VideoState *is, SDL_Event *event) {
    ...
    while (!SDL_PeepEvents(event, 1, SDL_GETEVENT, SDL_FIRSTEVENT, SDL_LASTEVENT)) 
    {
        if (!cursor_hidden && av_gettime_relative() - cursor_last_shown > CURSOR_HIDE_DELAY) 
        {
            SDL_ShowCursor(0);
            cursor_hidden = 1;
            FFP_LOG( FFP_INFO_DEBUG , "Mouse pointer OFF" );
        }
...
static int opt_frame_size(void *optctx, const char *opt, const char *arg)
{
    ...
    FFP_LOG( FFP_INFO_WARNING, "Option -s is deprecated, use -video_size.\n"); 
...
static int opt_format(void *optctx, const char *opt, const char *arg)
{
    ...
    if (!file_iformat) {
        ...
        FFP_LOG( FFP_INFO_ERROR, "Unknown input format: %s\n", arg); 
...
static int opt_frame_pix_fmt(void *optctx, const char *opt, const char *arg)
{
    ...
    FFP_LOG( FFP_INFO_WARNING, "Option -pix_fmt is deprecated, use -pixel_format.\n"); 
...
static int opt_sync(void *optctx, const char *opt, const char *arg)
{
    if (!strcmp(arg, "audio"))
        ...
        FFP_LOG( FFP_INFO_ERROR, "Unknown value for %s: %s\n", opt, arg ); 
        __exit(1);
    }
    return 0;
}
...
{
#if 0                         //disabled for GUI mode
    if (input_filename) {
        av_log(NULL, AV_LOG_FATAL,
               "Argument '%s' provided as input filename, but '%s' was already specified.\n",
                filename, input_filename);
        FFP_LOG( FFP_INFO_ERROR, "Argument '%s' provided as input filename, but '%s' was already specified.\n",filename, input_filename );                
        __exit(1);
    }
#endif    
    if (!strcmp(filename, "-"))
...
static int opt_codec(void *optctx, const char *opt, const char *arg)
{
   ...
   if (!spec) {
       av_log(NULL, AV_LOG_ERROR,
              "No media specifier was specified in '%s' in option '%s'\n",
               arg, opt);
       FFP_LOG( FFP_INFO_ERROR, "No media specifier was specified in '%s' in option '%s'\n",arg, opt );
       return AVERROR(EINVAL);
   }
   spec++;
   switch (spec[0]) {
   ...
   default:
       ...
       FFP_LOG( FFP_INFO_ERROR, "Invalid media specifier '%s' in option '%s'\n", spec, opt );       
       return AVERROR(EINVAL);
   }
   return 0;
}
   ...
   switch(op) {
      case AV_LOCK_CREATE:
          *mtx = SDL_CreateMutex();
          if(!*mtx) {
              av_log(NULL, AV_LOG_FATAL, "SDL_CreateMutex(): %s\n", SDL_GetError());
              FFP_LOG( FFP_INFO_WARNING, "SDL_CreateMutex(): %s\n", SDL_GetError());
              return 1;
          }
          return 0;
      case AV_LOCK_OBTAIN:
...
static void event_cli_loop(VideoState *cur_stream)
{
    ...
    for (;;) {
        ...
        case FF_QUIT_EVENT:
            isQuit = 1;
            FFP_LOG( FFP_INFO_NONE, "Request Quit (QUIT event)..." );
            do_exit(cur_stream);
            break;
        default:
            break;
        ...
        switch (event.type) 
        {
            case SDL_MOUSEMOTION:
                if (cursor_hidden) 
                {
                    SDL_ShowCursor(1);
                    cursor_hidden = 0;
                    FFP_LOG( FFP_INFO_DEBUG, "Mouse pointer ON" );
                }
                cursor_last_shown = av_gettime_relative();
                break;
            case SDL_WINDOWEVENT:
...
void EXPORTDLL multimedia_stream_start()
{
  ...
  switch(FFP_events->ui_type)
  {
    ...
    case FFP_GUI:
      ...
      break;
    default:
      FFP_LOG( FFP_INFO_WARNING, "STI_EVENT_TYPE is not defined. Set to default (STI_EVENT_CLI)");
      event_cli_loop(FFP_is);
      break;      
  }
...
{
  SDL_Event   event;
  event.type = FF_PAUSERESME_EVENT;
  SDL_PushEvent(&event);
}
...
int EXPORTDLL multimedia_init_device(FFP_EVENTS *ffp_events)
{
    ...
    if (SDL_Init (flags)) 
    {
        ...
        FFP_LOG( FFP_INFO_ERROR, "Could not initialize SDL - %s\n (Did you set the DISPLAY variable?)\n", SDL_GetError());        
        rtn = 1;
        return rtn;
    }
    FFP_LOG( FFP_INFO_DEBUG, "SDL Total Displays: %d\n", SDL_GetNumVideoDisplays());        
    ...
    if (av_lockmgr_register(lockmgr)) 
    {
        ...
        FFP_LOG( FFP_INFO_WARNING, "Could not initialize lock manager!\n");
        do_exit(NULL);
        rtn = 2;
        return rtn;
    }
    
    if ((FFP_events->screenID <= 0) && (FFP_events->ui_type==FFP_GUI))
    {
        FFP_LOG( FFP_INFO_ERROR, "Screen ID is empty!");
        do_exit(NULL); 
        rtn = 4;
        return rtn;
    }
	...
	{    
        ...
        if (window) {
            ...
            if (!renderer) 
            {
                ...
                FFP_LOG( FFP_INFO_WARNING, "Failed to initialize a hardware accelerated renderer: %s\n", SDL_GetError() );
            ...
            if (renderer) 
            {
                if (!SDL_GetRendererInfo(renderer, &renderer_info))
                {
                    ...
                    FFP_LOG( FFP_INFO_DEBUG, "Initialized %s renderer.\n", renderer_info.name );                    
                }
            }
        }
        if (!window || !renderer || !renderer_info.num_texture_formats) 
        {
            ...
            FFP_LOG( FFP_INFO_ERROR, "Failed to create window or renderer: %s", SDL_GetError() );
            do_exit(NULL);
            rtn = 3;
            return 3;
        }
...
{
      if ( (FFP_is) && (isFileOpen) )
      {
        ffp_vid_params.width  = FFP_is->width;
        ffp_vid_params.height = FFP_is->height;
        ffp_vid_params.Bpp    = 4;
...
static int runPlayThreadWithInit(void *ptr)
{
...
void EXPORTDLL multimedia_start_cli_player(int argc, char **argv, FFP_EVENTS *events)
{
    ...
    {
        FFP_LOG(FFP_INFO_ERROR, "\n");
        FFP_LOG(FFP_INFO_ERROR, "An input file must be specified\n");
        FFP_LOG(FFP_INFO_ERROR, "Use -h to get full help or, even better, run 'man %s'\n", "FFPlay");
        __exit(1);
    }
...
void EXPORTDLL multimedia_rgb_swap(void *pRGB, int width, int height, int bpp, int shiftR, int shiftG, int shitB)
{
    ...
    unsigned long wholePix, *pBuffer;
    unsigned char r,g,b;
    
    i = 0;
    pBuffer = (unsigned long*)pRGB;
    for (h = 0 ; h < height ; h++)
    {
        for (w = 0 ; w < width ; w++)
        {
          wholePix = pBuffer[i];
          r = (unsigned char)(wholePix>>shiftR);
          g = (unsigned char)(wholePix>>shiftG);
          b = (unsigned char)(wholePix>>shitB);
          wholePix = (((0x000000FF)&b)<<16)|(((0x000000FF)&g)<<8)|((0x000000FF)&r);
          pBuffer[i] = wholePix;
          i++;
        }  
    }  
...
void EXPORTDLL multimedia_test_screen( int xWinID, int latency ) 
{
   ...
   SDL_Renderer *test_renderer = NULL;

   SDL_Init(SDL_INIT_VIDEO);
   test_window   = SDL_CreateWindowFrom((void*)xWinID);
   test_renderer = SDL_CreateRenderer(test_window, -1, SDL_RENDERER_ACCELERATED);	 

   SDL_SetRenderDrawColor( test_renderer, 0, 0, 255, 0 );
   SDL_RenderClear( test_renderer );
   SDL_RenderPresent ( test_renderer );
   SDL_Delay( latency );
...
void EXPORTDLL SaveFramebufferAsPPM(void* Buff, int w, int h, int Bpp) 
{
	...
	if( t1-t > 2)
		...
		return;
#ifdef DEF_WIN
  sprintf(FileName,"./framebuffer_%d-win.ppm", idx++);
#else
